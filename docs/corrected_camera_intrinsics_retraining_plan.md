...
- Issue #25's D-PREFILTER investigation, PF-A/PF-B/PF-C, and supplemental
  ...
  Policy adoption is complete; handoff implementation and CUDA safety
  validation are not. Other unresolved owner/run contracts remain open.
...
P1 structure/fixed-input, P3 maximum-SH, P4 resolution/dimension, and P5 schedule
...
- **PF-B — input:** `renderer.temporal_prefilter` is a required string with
  ...
  Remaining nested required/allowed sets and runnable v1 are still incomplete.
...
manifest state, camera mathematics, seed, reporting, and publication lifecycle
...
- **A — explicit input and one origin-preserving transform:** require
  ...
  do not decide the still-open float32 representation and tolerance contract.
...
- D-OPT and related owners retain the loss time increment `0.1`, temporal LR,
  and normalization-change effects; D-SH/P0 retain active schedules and
...
subset change is not allowed. P3 now fixes the maximum temporal-SH
...
| config/run mode/provenance | Require effective `eval=True`, reject effective `eval=False`, apply the adopted strict nested JSON / immutable state and single from-scratch mode, and close its remaining field contract without legacy override authority. Reject legacy output/path reuse, capture the complete effective config, and replace executable config parsing where it reaches formal tooling. | Report path-remap and duplicate-basename ambiguity as bounded diagnostics. |
...
this documentation sync. The previously accepted P1/P2/P3/P4/P5 component CPU subsets are recorded
...
matching defaults. This full cross-entrypoint suite and integrated enforcement
do not yet exist; the accepted P1 fixed-input component subset is recorded in Step 5.
...
The accepted P1/P2/P3/P4/P5 component subset is recorded in Step 5; the remaining suite
...
The following are adopted future acceptance conditions, not tests performed
...
Gate A. Beyond the accepted P1/P2/P3/P4/P5 components in Step 5, the full pure resolver,
...
| independent formal entry / pure effective-state resolver | Sole owner of one explicit formal authority and bounded verified typed state, implemented through a lightweight formal-only bootstrap, stdlib-only resolver, and separately loaded heavy runtime. Initial CLI is config-locator-only and excludes `quiet`; legacy/general entry cannot bypass it. Resolve and reject before heavy import/JIT, run the read-only existing-output preflight, load and prepare the runtime without writers, then claim output exclusively immediately before the first writer. Bind approved semantic values and derived conditions but exclude runtime/digest/checkpoint/manifest/camera-math/seed/reporting/publication owners. | training and later evaluation/CUDA Reference consume the state; P0-A2 and manifest publish/verify rather than redefine it; local names/API/error structure remain CODEX discretion |
...
4. Integrate the approved one-path renderer invocation contract.
   ...
   **Complete in this document.** Integrate Issue #25's adopted PF-A/PF-B/PF-C
   ...
   maximum-SH, P4 resolution/dimension, and P5 schedule components and their CPU
   tests are implemented and accepted as recorded in Step 5 below. Initial-variance,
...
5. Implement the approved lightweight formal-only bootstrap, stdlib-only pure
   ...
   **Partial implementation accepted under Issue #39:** the P4 component
   ...
   This documentation sync records existing evidence without rerunning tests.
   ...
   Other objects, deeper nested paths, unknown spellings, and the full nested
   required/allowed sets remain unchecked, not allowed/adopted extensions.
   Actual-image/metadata agreement, formal owner handoff, image resize and
   camera/runtime integration remain unimplemented. Actual run divisor,
   interpolation/mask/alpha/depth and FoV-only decisions remain with D-DATA/P0-0.

   P2 parse data is not verified semantic state. P1/P3/P4/P5 leave the input
   unchanged, without filling, transforming, or rewriting it. P3/P4/P5 return detached frozen
   ...
   adapter or training connection exists. P6, D-TIME/initial-variance semantic
   ...
   not run. Step 5 as a whole and Gate A remain incomplete. Issue #39 still
...
- repository synchronization under Issue #26 of Issue #25's adopted
  D-PREFILTER policy and safety conditions. Implementation, numerical/CUDA
  validation, and checkpoint/manifest binding are not complete.
...
- Issue #35's bounded P2, Issue #36's bounded P1, Issue #37's bounded P3,
  Issue #38's bounded P5, and Issue #39's bounded P4 component implementation
  and CPU acceptance, as recorded in [Step 5](#phase-1-training-critical-foundation), not full formal
...
- D-PREFILTER handoff implementation, independent value/gradient and fresh-
  binary CUDA validation, and checkpoint/manifest identity binding under the
...
Beyond the accepted P1/P2/P3/P4/P5 partial components recorded in Step 5, their full
